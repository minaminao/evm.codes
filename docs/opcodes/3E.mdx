...
1. `offset`: コピーする最後に実行されたサブ[コンテキスト](/about)からの[リターンデータ](/about)内のバイトオフセット。
